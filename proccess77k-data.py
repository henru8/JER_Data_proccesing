...
for columns in df_440:
    ...
    print("first num",first_num,"and last num",last_num,"and diffrence",difer)
...
for columns in df_580:
    ...
    #Calculates the sum of the last value in a column (last_num) and adds this sum to each element in the column
    last_num = abs(df_580.iloc[-1, i_col])
    df_580.iloc[:, i_col] = df_580.iloc[:, i_col] + last_num

    i_rows = 0
    first_num = df_580.iloc[1, i_col]

    #loop to zero the measuerments
    while i_rows < 151 :
        df_580.iloc[i_rows, i_col] = (df_580.iloc[i_rows, i_col] - (((first_num*20)/(20+i_rows))))
        i_rows = i_rows +1
...
df_440.to_excel('dataExport_440.xlsx', index=True)
df_580.to_excel('dataExport_580.xlsx', index=True)
